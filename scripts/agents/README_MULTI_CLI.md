...
3. **Global config default** (`agent.provider` in automation.yaml)
4. **Default to Claude**
...
scripts/ami-run scripts/automation/tasks.py
```
...
scripts/ami-run -m pytest tests/unit/test_agent_cli.py::TestClaudeModels -xvs
scripts/ami-run -m pytest tests/unit/test_agent_cli.py::TestGeminiModels -xvs

# Run multi-provider factory tests
scripts/ami-run -m pytest tests/unit/test_agent_cli.py::TestMultiProviderFactory -xvs

# Run GeminiAgentCLI tests
scripts/ami-run -m pytest tests/unit/test_agent_cli.py::TestGeminiAgentCLI -xvs
```
...
scripts/ami-run -m pytest tests/integration/test_multi_provider_e2e.py -xvs
```
