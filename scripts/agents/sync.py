...
class SyncExecutor:
    ...
    def sync_module(self, module_path: Path, user_instruction: str | None = None) -> SyncResult:
        ...
        while time.time() - start_time < timeout:
            ...
            logger.info("worker_attempt", module=module_name, attempt=attempt_num, session_id=self.session_id)
...
"""
            ...
            if "WORK DONE" in worker_output:
                ...
                logger.info("worker_feedback", reason=feedback_reason, session_id=self.session_id)
            else:
...
"""
            ...
            if "PASS" in moderator_output:
                ...
                logger.info("sync_success", module=module_name, attempts=attempt_num, session_id=self.session_id)
            ...
            if "FAIL:" in moderator_output:
                ...
                logger.info("moderator_fail", reason=fail_reason, session_id=self.session_id)
                additional_context = f"\n\n## Previous Attempt Failed\n\n{fail_reason}\n\nPlease fix and retry.\n"
                # Continue to next attempt
            else:
                logger.warning("moderator_unexpected_output", output=moderator_output[:200], session_id=self.session_id)
        ...
        # Timeout
        total_duration = time.time() - start_time
        logger.error("sync_timeout", module=module_name, attempts=attempt_num, session_id=self.session_id)
        return SyncResult(
            module_path=module_path,
            status="timeout",
            attempts=attempts,
