#!/usr/bin/env bash
""":'
exec "$(dirname "$0")/ami-run" "$(dirname "$0")/audit_shebangs.py" "$@"
"""
...
Usage:
    scripts/agents/validation/audit_shebangs.py                          # Check all Python files
    scripts/agents/validation/audit_shebangs.py --fix                    # Fix incorrect shebangs
    scripts/agents/validation/audit_shebangs.py --verbose                # Show all files checked
    scripts/agents/validation/audit_shebangs.py -d /path/to/dir          # Check specific directory
...
CORRECT_AMI_RUN_BASE = '#!/usr/bin/env bash\n"""\'exec "$(dirname "$0")/../scripts/ami-run"'


def _load_shebang_patterns() -> dict[str, Any]:
    """Load shebang patterns from YAML config.

    Returns:
        Dictionary containing incorrect and security patterns
    """
    config = get_config()
    patterns_dir = config.root / "scripts/config/patterns"
    patterns_file = patterns_dir / "shebang_patterns.yaml"

    if not patterns_file.exists():
        # Return empty dict if config doesn't exist
        return {"incorrect_patterns": [], "security_patterns": []}
...
SHEBANG_CONFIG = _load_shebang_patterns()

# Incorrect patterns to detect (from config)
INCORRECT_PATTERNS = [
    (pattern_config.get("pattern", "").encode(), pattern_config.get("description", "")) for pattern_config in SHEBANG_CONFIG.get("incorrect_patterns", [])
]

# Security risk patterns (from config)
SECURITY_PATTERNS = [
    (pattern_config.get("pattern", "").encode(), pattern_config.get("description", "")) for pattern_config in SHEBANG_CONFIG.get("security_patterns", [])
]
...
def check_shebang(file_path: Path, verbose: bool = False) -> tuple[bool, str]:
    ...
    has_ami_run = any("ami-run" in line for line in (line1, line2, line3))
    if has_ami_run:
...
def get_correct_shebang(file_path: Path) -> tuple[str, str]:
    ...
    """
    # Determine relative path to ami-run
    try:
        ...
        if depth == 1:
            ...
            ami_run_path = '"$(dirname "$0")/scripts/ami-run"'
            script_ref = '"$0"'
        else:
            # File is deeper
            parent_dirs = "../" * (depth - 1)
            ami_run_path = f'"$(dirname "$0")/{parent_dirs}scripts/ami-run"'
            script_ref = '"$0"'
...
def audit_repository(
    ...
    if staged_only:
        ...
        if not python_files:
            ...
            return 0
    # Only scan specific directories when running from repository root
    elif scan_dir is None and target_dir == ROOT:  # Default case - scanning from root
        python_files = []
        for subdir in ["scripts", "backend", "tests"]:
...
def main() -> int:
    ...
    try:
        ...
        if issue_count > 0 and not args.fix:
            ...
            if args.directory:
                logger.info(f"  scripts/agents/validation/audit_shebangs.py --directory {args.directory} --fix")
            else:
                logger.info("  scripts/agents/validation/audit_shebangs.py --fix")
