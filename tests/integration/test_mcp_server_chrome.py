#!/usr/bin/env python
"""Integration tests for Chrome MCP server run script."""
...
class TestChromeMCPServer:
    """Test the Chrome MCP server run script with full environment setup."""
    ...
    def setup(self):
        ...
        self.venv_dir = self.browser_dir / ".venv"
        self.run_script = self.browser_dir / "backend" / "mcp" / "chrome" / "run_chrome.py"
    ...
    def test_chrome_mcp_server_setup_and_run(self):
        """Test that the Chrome MCP server run script sets up environment and starts correctly."""
        ...
        env["CHROME_HEADLESS"] = "true"  # Run in headless mode for testing
        ...
        try:
            ...
            requirements_file = self.browser_dir / "requirements.txt"
    ...
    def test_chrome_mcp_server_browser_tools(self):
        """Test that the Chrome MCP server provides browser tools."""
        # Start the process
        process = subprocess.Popen(
            [sys.executable, str(self.run_script), "--stdio"],
            stdin=subprocess.PIPE,
        ...
        try:
            ...
            # The server should respond with available browser tools
            # like browser_launch, browser_navigate, etc.
    ...
    def test_chrome_mcp_server_headless_mode(self):
        """Test that the Chrome MCP server can run in headless mode."""
        ...
        try:
            ...
            # Send browser launch request
            launch_request = {"jsonrpc": "2.0", "method": "tools/call", "params": {"name": "browser_launch", "arguments": {"headless": True}}, "id": 1}

            process.stdin.write(json.dumps(launch_request) + "\n")
            process.stdin.flush()

            # The server should be able to launch a headless browser
