...
from orchestrator.models.bpmn_models import Process, Task, Agent, ProcessStatus, TaskStatus, AgentStatus, TaskType

@pytest_asyncio.fixture
async def dgraph_client_fixture():
    client = DgraphClient()
...
async def test_upsert_and_get_process(dgraph_client_fixture):
    ...
    # Upsert the process
    uid = await dgraph_client_fixture.upsert_process(process)
    assert uid is not None
    
    # Get the process and verify
    retrieved_process = await dgraph_client_fixture.get_process(process_id)
...
async def test_update_process_status(dgraph_client_fixture):
    ...
    await dgraph_client_fixture.upsert_process(process)

    # Update status
    updated = await dgraph_client_fixture.update_process_status(process_id, ProcessStatus.COMPLETED)
...
async def test_upsert_and_get_task(dgraph_client_fixture):
    ...
    # Upsert the task
    uid = await dgraph_client_fixture.upsert_task(task)
    assert uid is not None
    
    # Get the task and verify
    retrieved_task = await dgraph_client_fixture.get_task(task_id)
...
async def test_update_task_status(dgraph_client_fixture):
    ...
    await dgraph_client_fixture.upsert_task(task)

    # Update status
    updated = await dgraph_client_fixture.update_task_status(task_id, TaskStatus.IN_PROGRESS)
    assert updated is True
...
async def test_upsert_and_get_agent(dgraph_client_fixture):
    ...
    # Upsert the agent
    uid = await dgraph_client_fixture.upsert_agent(agent)
    assert uid is not None
    
    # Get the agent and verify
    retrieved_agent = await dgraph_client_fixture.get_agent(agent_id)
...
async def test_update_agent_status(dgraph_client_fixture):
    ...
    await dgraph_client_fixture.upsert_agent(agent)

    # Update status
    updated = await dgraph_client_fixture.update_agent_status(agent_id, AgentStatus.BUSY)
    assert updated is True
...
async def test_get_process_instances(dgraph_client_fixture):
    ...
    await dgraph_client_fixture.upsert_process(process1)
    await dgraph_client_fixture.upsert_process(process2)
    await dgraph_client_fixture.upsert_process(process3)

    # Get all instances for the definition
    instances_all = await dgraph_client_fixture.get_process_instances(definition_id)
    ...
    instances_v1 = await dgraph_client_fixture.get_process_instances(definition_id, version=1)
    ...
    instances_v2 = await dgraph_client_fixture.get_process_instances(definition_id, version=2)
    assert len(instances_v2) == 1
    assert any(p.id == "proc_inst_3" for p in instances_v2)

    # Test with non-existent definition
    instances_non_existent = await dgraph_client_fixture.get_process_instances("non_existent_def")
    assert len(instances_non_existent) == 0

    # Test with non-existent version
    instances_non_existent_version = await dgraph_client_fixture.get_process_instances(definition_id, version=99)
